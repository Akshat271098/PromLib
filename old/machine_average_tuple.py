...
class machine_average_tuple:
    ...
    def memory(self):
        query=' sum_over_time(node_memory_MemAvailable_bytes{{ instance={}  }}[{}]) /((10*60/15)*(1024^3))'.format(self.instance,self.window)
        #print(query)
        response = requests.get(self.prometheus + '/api/v1/query', params={
            'query': query})
        #print(response.json())
    ...
    def disk(self):
        query='sum_over_time(node_filesystem_avail_bytes{{ instance={},device!~"rootfs",mountpoint="/" }}[{}]) /((10*60/15)*(1024^3))'.format(self.instance,self.window)
        #print(query)
        response = requests.get(self.prometheus + '/api/v1/query', params={
            'query': query})
        #print(response.json())
